from database import SessionLocal, Product, UploadedFile, Sale, Supply, Balance, Spoil, Parameter, CachedForecast, CachedIdealStock, Supplier, SupplierItem, ApproximatePrice
...
def save_parsed_data(df, filename):
    ...
    try:
        ...
        for _, row in work_df.iterrows():
            ...
            # Add, update or delete sale based on source-file priority.
            ...
            # Add, update or delete supply based on source-file priority.
            ...
            # Add or update balance based on source-file priority.
    ...
    try:
        ...
        for sale, sku in sales_query:
            sales_data.append({
                'sku': sku,
                'date': sale.date,
                'outbound': float(sale.quantity or 0),
            })
        ...
        for spoil, sku in spoils_query:
            spoils_data.append({
                'sku': sku,
                'date': spoil.date,
                'spoil_qty': float(spoil.quantity or 0),
            })
    ...
    try:
        ...
        for spoil, sku in query:
            data.append({
                'sku': sku,
                'date': spoil.date,
                'quantity': float(spoil.quantity or 0),
                'reason': spoil.reason,
            })
...
def get_current_stock():
    session = get_session()
    try:
        # Optimized query using window function for latest balance per product
        from sqlalchemy import func
        ...
        data = []
        for product_id, balance, sku in query:
            data.append({
                'sku': sku,

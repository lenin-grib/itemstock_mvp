...
Ниже краткая последовательность работы в интерфейсе.

### 1. Откройте приложение

- Перейдите в браузере на адрес приложения (локально обычно http://localhost:8501).
...
- Загрузите один или несколько файлов логов (xlsx).
- Загрузите файл списаний (xlsx).
- После загрузки дождитесь обновления таблиц (приложение автоматически пересчитывает данные).
...
- Просмотрите таблицу прогноза по SKU.
...
### 4. Загрузите данные поставщиков и прайс-лист

Во вкладке Поставщики:

- Загрузите файл поставщиков.
- Загрузите прайс-лист.
- Проверьте, что появились поставщики, сроки/стоимость доставки и количество позиций.

### 5. Настройте параметры расчета

Во вкладке Параметры:

- Установите коэффициент запаса.
- Установите минимальный остаток.
- Установите период тренда в неделях.

После изменения параметров пересчитайте прогноз/идеальный сток (если требуется).

### 6. Сформируйте рекомендации по заказу

Во вкладке Склад и заказы:

- Просмотрите идеальный сток и расчет количества к заказу на 1/2/3/4 недели.
- Сформируйте рекомендации по поставщикам.
- Проверьте предупреждения о минимальной сумме заказа и позиции без поставщика.

### 7. Что делать при проблемах

- Если файл не загружается: проверьте формат колонок и структуру Excel согласно разделу Data Inputs.
- Если в прогнозе нули: проверьте, что в логах есть валидные SKU и даты, а также загружены списания.
- Если нет рекомендаций по поставщику: проверьте, что SKU присутствует в прайс-листе и сопоставлен с поставщиком.
